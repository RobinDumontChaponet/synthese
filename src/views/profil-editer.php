	...
	<section id="info">
		...
		<dl>
			...
			<dt id="sexe<?php echo strtoupper($ancien->getSexe());?>"><label for="inputSex">Sexe</label></dt>
				...
				<dd><?php echo ($_SESSION['user_auth']['write'])?'<input id="inputBirthday" name="birthday" type="date" placeholder="Pas de date de naissance" maxlength="10" value="'.$ancien->getDateNaissance().'"/>':'<span>'.$ancien->getDateNaissance().'</span>';?></dd>
		...
		<section id="entreprises">
			...
			<ul>
				<?php if($entreprises != NULL) { // Il faudra faire quelque chose pour pouvoir les modifiers, soit là, soit sur une autre page
					...
					<li>
						<a class="edit" href="entreprise-modifier/<?php echo $ancien->getId(); ?>&<?php echo $entreprise->getEntreprise()->getId();?>&<?php echo $entreprise->getDateEmbaucheDeb()?>&<?php if($entreprise->getDateEmbaucheFin() == 0000-00-00) echo '0000-00-00'; else echo $entreprise->getDateEmbaucheFin()?>&<?php echo $entreprise->getPoste()->getId();?>">Éditer</a><a class="delete" href="index.php?requ=entreprise-selectionner-supprimer&idEntreprise=<?php echo $entreprise->getEntreprise()->getId();?>&idAncien=<?php echo $ancien->getId(); ?>&idPoste=<?php echo $entreprise->getPoste()->getId(); ?>&dateDebut=<?php echo $entreprise->getDateEmbaucheDeb(); ?>">Supprimer</a>
						...
						<dl>
							...
							<dt class="periode">Période</dt>
							<dd>Du <?php echo strftime('%d %B %Y', strtotime($entreprise->getDateEmbaucheDeb()));?><?php if($entreprise->getDateEmbaucheFin() == NULL || $entreprise->getDateEmbaucheFin() == 0000-00-00) echo ' à maintenant'; else echo ' au '.strftime('%d %B %Y', strtotime($entreprise->getDateEmbaucheFin()));?></dd>
						</dl>
					</li>
					<?php }
				} ?>
