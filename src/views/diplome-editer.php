	...
	<form action="diplome-editer/<?php echo $diplome->getId()?>" method="post">
		<article>
			...
			<dl>
				...
				<dd class="domaine"><select id="domaineLibelle" name="domaineLibelle"><?php foreach ($domaines as $domaine) {echo '<option'.(($domaine->getId() == $diplome->getDomaine()->getId())?' selected':'').' value="'.$domaine->getId().'">'.$domaine->getLibelle().'</option>';}?></select><a href="domaines" target="_blank">Voir tout les domaines (nouvel onglet)</a></dd>
