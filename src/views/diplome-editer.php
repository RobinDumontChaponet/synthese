	...
	if (isset($diplome) && $diplome != NULL && $_SESSION['user_auth']['write']) { ?>
		<form action="diplome-editer/<?php echo $diplome->getId()?>" method="post">
			<ul>
				...
				<li><label for="domainLibelle">Domaine</label><select id="domainLibelle" name="domainLibelle"><?php foreach ($domaines as $domaine) {echo '<option'.(($domaine->getId() == $diplome->getDomaine()->getId())?' selected':'').' value="'.$domaine->getId().'">'.$domaine->getLibelle().'</option>';}?></select></li>
