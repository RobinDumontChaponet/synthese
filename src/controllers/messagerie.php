...
for($i = (count($nm)-1); $i >= 0; $i--)
    $nouveauxMessages .= '<tr><td>'.$nm[$i]->getExpediteur()->getNom().' '.$nm[$i]->getExpediteur()->getPrenom.' </td>
...
for($i = (count($messagesLus)-1); $i >= 0; $i--)
    $anciensMessages .= '<tr><td>'.$messagesLus[$i]->getExpediteur()->getNom().' '.$messagesLus[$i]->getExpediteur()->getPrenom.' </td>
